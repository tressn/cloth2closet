...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const project = await prisma.project.findUnique({
    where: { id: projectId },
    select: { id: true, dressmakerId: true, dressmaker: { select: { userId: true } } },
  });

  if (!project) return bad("Project not found", 404);

  const isAdmin = session.user.role === "ADMIN";
  const isOwner = project.dressmaker?.userId === session.user.id;
  ...
  const updated = await prisma.projectDetails.upsert({
    ...
    create: { projectId, dressmakerPrivateNotes: notes },
    select: { dressmakerPrivateNotes: true, updatedAt: true },
  });
